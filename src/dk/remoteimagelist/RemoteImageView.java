...
import dk.remoteimagelist.image.RemoteImageLoader;
import dk.remoteimagelist.image.RemoteImageLoaderHandler;
...
public class RemoteImageView extends ViewSwitcher {
     ...
     * class.
     * 
     * @param imageLoader the shared image loader
     */
     ...
     * @param autoLoad Whether the download should start immediately after
     *            creating the view. If set to false, use {@link #loadImage()}
     *            to manually trigger the image download.
     */
     ...
     *            {@link ProgressBar} which is displayed while the image is
     *            loading
     * @param autoLoad Whether the download should start immediately after
     *            creating the view. If set to false, use {@link #loadImage()}
     *            to manually trigger the image download.
     */
    public RemoteImageView(Context context, String imageUrl, Drawable progressDrawable, boolean autoLoad) {
        super(context);
     ...
     *            to manually trigger the image download.
     */
    public RemoteImageView(Context context, String imageUrl, Drawable progressDrawable, Drawable errorDrawable, boolean autoLoad) {
        super(context);
    ...
    public RemoteImageView(Context context, AttributeSet attributes) {
        ...
        int progressDrawableId = attributes.getAttributeResourceValue(Ignition.XMLNS, ATTR_PROGRESS_DRAWABLE, 0);
        int errorDrawableId = attributes.getAttributeResourceValue(Ignition.XMLNS, ATTR_ERROR_DRAWABLE, DEFAULT_ERROR_DRAWABLE_RES_ID);
        int defaultDrawableId = attributes.getAttributeResourceValue(Ignition.XMLNS, ATTR_DEFAULT_DRAWABLE, DEFAULT_DRAWABLE_RES_ID);
        ...
        boolean autoLoad = attributes.getAttributeBooleanValue(Ignition.XMLNS, ATTR_AUTO_LOAD, true);

        initialize(context, imageUrl, progressDrawable, errorDrawable, defaultDrawable, autoLoad);
        // styles.recycle();
    }

    private void initialize(Context context, String imageUrl, Drawable progressDrawable, Drawable errorDrawable, Drawable defaultDrawable,
            boolean autoLoad) {
    ...
    public void setDefaultDrawable(int imageResourceId) {
        defaultDrawable = getContext().getResources().getDrawable(imageResourceId);
    }
    ...
    /**
     * The drawable that is shown in place of the downloaded image. It will be
     * replaces once the image has been downloaded. This can be used to display
     * a placeholder image while the download is in progress. Corresponds to the
     * view attribute ignition:defaultDrawable. If left blank, no placeholder
     * image will be used.
     * 
     ...
     * the image. Corresponds to the view attribute ignition:progressDrawable.
     * If left blank, the platform's standard indeterminate progress drawable
     * will be used.
     * 
    ...
    public void setPaused(boolean paused) {
        if (mPaused != paused) {
            mPaused = paused;
            if (!paused && !TextUtils.isEmpty(imageUrl)) {
                loadImage();
            }
        }
    }
